...
class DownBlock(nn.Module):
    def __init__(self, in_ch, out_ch, p_drop=0.0):
        super().__init__()
        # Match their naming exactly: down_block = Sequential(0: MaxPool2d, 1: ConvBlock)
        self.down_block = nn.Sequential(
            nn.MaxPool2d(2),
            ConvBlock(in_ch, out_ch, p_drop)
        )
...
class UpBlock(nn.Module):
    ...
    def forward(self, x, skip):
        x = self.up(x)
        # pad for odd sizes, just in case
...
class UNet(nn.Module):
    def __init__(self, in_ch=1, num_classes=2,
        ...
        # out_conv likely has bias=True in TS; keep bias=True
        self.out_conv = nn.Conv2d(d5, num_classes, kernel_size=1, bias=True)

        # NOTE: we intentionally DO NOT add Softmax here for training.
